...
const ios: sekure = new sekure(
  'https://api.sekure.com.co/OSStage',
  'ec151c0d3e6648f88b4a65fc3a0b1ab1'
);
  ...
  Parameters: [
    {
      ...
      InputParameterDescription:
        'Fecha de nacimiento del tomador, la edad máximo es de 70 años y el formato de fecha correcto es AÑO-MES-DÍA',
    ...
    },
    {
      ...
      InputParameterDescription:
        'Se debe de enviar los valores. 1 = Si es semestral, 2 = Si es anual',
      InputParameterRequired: 'true',
      ShowApi: true,
      InputParameterSchemaList: [],
    },
...
test('It should bring a successful confirm', async () => {
  ...
  myMock.mockResolvedValue(resp);
  return await ios
    .Confirm(request, 'fc78fa74-8b7a-4a6b-9044-45ec5e15e695')
    .then(data => expect(data).toEqual(product));
});
...
test('It should bring a successful emit', async () => {
  ...
  myMock.mockResolvedValue(resp);
  return await ios
    .Emit(request, 'fc78fa74-8b7a-4a6b-9044-45ec5e15e695')
    .then(data => expect(data).toEqual(product));
});
...
test('It should bring a successful confirm by lot', async () => {
  ...
  myMock.mockResolvedValue(resp);
  return await ios
    .ConfirmLot(requestLot, 'fc78fa74-8b7a-4a6b-9044-45ec5e15e695')
    .then(data => expect(data).toEqual(product));
});
...
test('It should bring a successful emit by lot', async () => {
  ...
  myMock.mockResolvedValue(resp);
  return await ios
    .EmitLot(requestLot, 'fc78fa74-8b7a-4a6b-9044-45ec5e15e695')
    .then(data => expect(data).toEqual(product));
});
...
test('It should bring a successful askSekure', async () => {
  ...
  myMock.mockResolvedValue(resp);
  return await ios
    .AskSekure(requestAskSekure, 30, 'test')
    .then(data => expect(data).toEqual(product));
});
