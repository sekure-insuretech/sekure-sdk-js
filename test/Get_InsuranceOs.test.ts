...
test('It should bring the estimate by the sessionId', async () => {
  ...
  myMock.mockResolvedValue(resp);
  return await ios
    .GetEstimateBySessionId('fc78fa74-8b7a-4a6b-9044-45ec5e15e695')
    .then(data => expect(data).toEqual(product));
});
...
test('It should bring a quote for the sessionId', async () => {
  ...
  myMock.mockResolvedValue(resp);
  return await ios
    .GetQuoteBySessionId('fc78fa74-8b7a-4a6b-9044-45ec5e15e695')
    .then(data => expect(data).toEqual(product));
});
...
test('It should bring a stage for the sessionId', async () => {
  ...
  myMock.mockResolvedValue(resp);
  return await ios
    .GetProductStage('fc78fa74-8b7a-4a6b-9044-45ec5e15e695')
    .then(data => expect(data).toEqual(product));
});
...
test('It should bring the payment gateway configuration', async () => {
  ...
  myMock.mockResolvedValue(resp);
  return await ios
    .GetPaymentGatewayConfiguration('test', 'test')
    .then(data => expect(data).toEqual(product));
});
