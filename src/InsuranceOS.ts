...
export default class InsuranceOS {
  ...
  public ConfirmLot = async (request: t.ExecutatbleProductLot, sessionId: string) : Promise<i.QuoteProductLot> => {
    const { data } = await axios.post<i.QuoteProductLot>(`${this._url}/Products/Batch/Confirm/${sessionId}`, JSON.stringify(request), this.ConfigRequest(this._skrKey));
    return data;
  };

  public EmitLot = async (request: t.ExecutatbleProductLot, sessionId: string) : Promise<string> => {
    const { data } = await axios.post<string>(`${this._url}/Products/Batch/Emit/${sessionId}`, JSON.stringify(request), this.ConfigRequest(this._skrKey));
    return data;
  };

  public Pay = async (request: t.PaymentDetail) : Promise<string> => {
    const { data } = await axios.post<string>(`${this._url}/Payment`, JSON.stringify(request), this.ConfigRequest(this._skrKey));
    return data;
  };

  public GetPaymentStatus = async (request: t.PaymentDetail) : Promise<i.PaymentStatus> => {
    const { data } = await axios.post<i.PaymentStatus>(`${this._url}/Payment/Status`, JSON.stringify(request), this.ConfigRequest(this._skrKey));
    return data;
  };

  public GetPaymentGatewayConfiguration = async (paymentGatewayName: string, productName: string) : Promise<i.PaymentGatewayProduct> => {
    const { data } = await axios.get<i.PaymentGatewayProduct>(`${this._url}/Payment/Configuration/paymentGatewayName=${paymentGatewayName}&productName=${productName}`, this.ConfigRequest(this._skrKey));
    return data;
  };

  public AskSekure = async (parameters: any, productId: number, productName:string) : Promise<string> => {
    const { data } = await axios.post<string>(`${this._url}/Products/AskSekure/productId=${productId}&productName=${productName}`, 
    JSON.stringify(parameters), this.ConfigRequest(this._skrKey));
    return data;
  };
}